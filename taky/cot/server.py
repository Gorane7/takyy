...
class COTServer:
    ...
    def __init__(self):
        ...
        """
        self.allowed_ips = config.get("custom", "allowed_ips")
        self.blocked_ips = config.get("custom", "blocked_ips")
    ...
    def srv_accept(self, srv_sock, force_tcp=False, mon_client=False):
        ...
        if mon_client:
            ...
            if "all" not in self.allowed_ips and ip_addr not in self.allowed_ips:
                self.lgr.info("Attempted %s cot client connection from %s:%s, but that IP was not allowed", stype, ip_addr, port)
                self.lgr.info(f"ALlowed ips: {self.allowed_ips}")
                return
            if ip_addr in self.blocked_ips:
                self.lgr.info("Attempted %s cot client connection from %s:%s, but that IP was blocked", stype, ip_addr, port)
                return
